...
if MODE == "live":
    if not API_KEY or not API_SECRET or not API_PASSPHRASE:
        raise ValueError("Missing KuCoin API credentials in LIVE mode")
...
def fetch_funding_history(exchange, symbol, since_ms, until_ms):
    ...
    df["timestamp"] = pd.to_datetime(df["ts"], unit="ms").dt.tz_localize(None)
...
def filter_recent_fvgs(fvgs, current_ts, htf_timeframe, max_age_bars=FVG_MAX_AGE_BARS):
    ...
    tf = tf_minutes.get(htf_timeframe.lower(), 15)
    max_age_seconds = max_age_bars * tf * 60
...
def price_in_zone(price, lo, hi, buffer_pct=RETEST_BUFFER_PCT):
...
def has_rejection_wick(c, direction="bull", wick_body_ratio=WICK_BODY_RATIO):
    body = abs(c["Close"] - c["Open"])
    if body == 0:
        return False
    if direction == "bull":
        lw = (c["Open"]-c["Low"]) if c["Open"]>=c["Close"] else (c["Close"]-c["Low"])
        return lw/body >= wick_body_ratio
    else:
        uw = (c["High"]-c["Open"]) if c["Open"]<=c["Close"] else (c["High"]-c["Close"])
        return uw/body >= wick_body_ratio

# ======================================================
# STATE MANAGEMENT (TZ FIXED)
# ======================================================
...
def process_bar(symbol, etf_df, htf_df, state, exchange=None, funding_series=None):
    ...
    ts = etf.index[i].replace(tzinfo=None)
    ...
    if dd >= MAX_DRAWDOWN and state["position"]!=0:
        ...
        posv = abs(exit_price * state["entry_size"])
        pnl = gross - posv*SLIPPAGE_RATE - posv*FEE_RATE

        state["capital"] += pnl

        row = {
            "Symbol":symbol,
            "Entry_DateTime":state["entry_time"],
            "Exit_DateTime":ts,
            "Position":"Long" if state["position"]==1 else "Short",
        ...
        state.update({
            "position":0,"entry_price":0.0,"entry_sl":0.0,"entry_tp":0.0,
            "entry_time":None,"entry_size":0.0
        })
        return state, row
    ...
    if state["position"]!=0:
        ...
        if exit_flag:
            ...
            state.update({
                "position":0,"entry_price":0.0,"entry_sl":0.0,
    ...
    if state["position"] == 0:
        ...
        if COOLDOWN_HOURS>0 and state.get("last_exit_time") is not None:
            last_exit = state["last_exit_time"].replace(tzinfo=None)
            if (ts - last_exit).total_seconds()/3600 < COOLDOWN_HOURS:
        ...
        if signal!=0 and (not USE_ATR_STOPS or (USE_ATR_STOPS and etf["ATR"].iloc[i] > 0)):
            ...
            if risk > 0:
                ...
                if size > 0:
                    ...
                    posv = abs(entry_price_used * size)
                    state["capital"] -= posv*SLIPPAGE_RATE
                    state["capital"] -= posv*FEE_RATE
...
def worker(symbol):
    ...
    while True:
        try:
            ...
            closed_ts = etf.index[-2].replace(tzinfo=None)
...
def generate_daily_summary():
    try:
        ...
        # convert to UTC naive to match your stored trade timestamps
        ...
        lines = [
            f"📊 FUTURES DAILY SUMMARY — {now_ist.strftime('%Y-%m-%d %I:%M %p IST')}",
            "-"*60
        ]
        ...
        for sym in SYMBOLS:
            ...
            if os.path.exists(trades_csv):
                ...
                if len(df):
                    ...
                    df["Exit_DateTime"] = df["Exit_DateTime"].dt.tz_localize(None)
            ...
            lines.append(
                f"{sym}: cap ${cap:,.2f} ({roi:+.2f}%) | "
                f"today {n_trades_today} trades, {wins_today} wins | "
                f"PnL ${pnl_today_sym:+.2f} | all WR {wr:.1f}%"
            )
        ...
        lines.append("-"*60)
        lines.append(
            f"TOTAL: cap ${total_cap:,.2f} ({port_roi:+.2f}%) | "
            f"today {n_today} trades | WR {wr_today:.1f}% | PnL ${pnl_today:+.2f}"
        )
...
def summary_scheduler():
    ...
    while True:
        try:
            now = ist_now()
            if now.hour == SUMMARY_HOUR_IST:
                if last_sent_date is None or last_sent_date != now.date():
...
Min Confluence: {MIN_CONFLUENCE}/5
ATR SL: {ATR_MULT_SL}×ATR | RR {RR_FIXED}
"""
